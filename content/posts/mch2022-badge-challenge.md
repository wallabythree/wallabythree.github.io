...
"Hack Me If You Can" comes preinstalled on the badge. A copy of 
`hack_me_if_you_can.elf` could also be downloaded from the 
[CTF website](https://ctf.mch2022.org) for the duration of the competition.
Although the CTF has ended, you can still exploit this vulnerability on your
own badge.
...
`do_echo_recursive()`. This function calls itself 45 times before calls into the
actual echo function, `do_echo()`. We said earlier that the register window
...
As we can see below, the exploit successfully retrieves the placeholder flag
from our own badge. A CTF organiser has also kindly confirmed the exploit works
against badges that contain the real flag.
